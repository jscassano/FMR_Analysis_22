```{r}
flightfull$markers[95]<-"" 
flightfull$markers[99]<-"" 
flightfull$markers[123]<-"" 
flightfull$markers[134]<-"" 
```
...
flightfull.sub<-flightfull%>%
  filter(Seconds > chbr1+120 & Seconds < chbr1+(60*25)) # make "usetime" 2 min to 10 min

#Base R plot for CO2
plot(flightfull$Seconds[fly], flightfull$VCO2[fly], main = "VCO2", type="l", xlab = "Seconds flight", ylab = "VCO2 flight") #subsets "just flight" points
...
##Calculate summary statistics for flight time
flightfull<-flightfull%>%
  filter(Seconds >= chbr1+120 & Seconds <= chbr1+(60*25)) # make "usetime" 2 min to 10 min
